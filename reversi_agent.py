...
############################# JADE I START ##################################################
class JADE_I_Agent(ReversiAgent):
    def __index__(self):
...
################################## JADE II START ##########################################################
class JADE_II_Agent(ReversiAgent):
    ...
    def search(self, color, board, valid_actions, output_move_row, output_move_column):
        try:
            evaluation, best_state = self.Min_max(board, valid_actions, 2, 0, -9900, 9900, True)
    ...
    def Min_max(self, board: np.array, validactions: np.array, depth: int, level: int, alpha: int, beta: int,
                gain: bool):
        if depth == 0:
        ...
        MaxAlpha: int = alpha
        maxEvaluation: int = -9900
        player: int = self._color

        for action in validactions:
            newstate, newaction = self.createState(board, action, player)
            newmove = self.Min_min(newstate, newaction, depth - 1, level + 1, MaxAlpha, beta, not gain)
    ...
    def Min_min(self, board: np.array, validactions: np.array, depth: int, level: int, alpha: int, beta: int,
                gain: bool):
        if depth == 0:
            return self.evaluation(board)

        MinBeta: int = beta
        minEvaluation: int = 9900
        Player: int = self.getOpponent(self._color)
        for action in validactions:
            newstate, newaction = self.createState(board, action, Player)
            newmove = self.Min_max(newstate, newaction, depth - 1, level + 1, alpha, MinBeta, not gain)
    ...
    ################################# JADE II END #############################################################
################################## JADE III START ########################################################
class JADE_III_Agent(ReversiAgent):
    def __index__(self):
        super(QewAgent, self)
...
class JADE_IV_Agent(ReversiAgent):
    ...
    def evaluate_score(self, board, player, opp_player):
        ...
        for i in range(8):
            for j in range(8):
                sum += board[i][j] * select_stage[i][j]
        return  sum
...
class JADE_V_Agent(ReversiAgent):
    def search(
            self, color, board, valid_actions,
            output_move_row, output_move_column):
        try:
